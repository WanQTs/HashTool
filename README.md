...
每行 `哈希值  文件名`（空格或制表符分隔均可），支持 `#`/`;` 注释与 `MD5 (文件名) = 哈希值`（certutil）格式；文件名可含相对子目录，并支持 GNU 转义文件名（`\\`、`\n`、`\r`）。无法识别的行会以「格式错误」标出，不会静默忽略；`.md5`/`.sha1`/`.sha256`/`.sha512` 扩展名与哈希长度推断的算法不符时会提示确认。安全边界：清单中的文件名必须解析在所选目标目录之内，`../` 或外部绝对路径会以错误标出，不会读取目录外的文件。
</details>

<details>
<summary><b>文件数量特别多（上万）时卡顿？</b></summary>

任务会一次性提交到后台线程池（最多 4 线程并行），表格无虚拟滚动，万级以上文件可能出现内存与界面开销上升；属已知限制，建议按目录分批处理。
</details>
