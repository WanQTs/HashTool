...
_STRINGS: dict[str, dict[str, str]] = {
    ...
    "bt_hint_format": {"zh": "清单格式：每行“哈希值  文件名”（兼容 MD5Sum/SHA256SUM 格式），支持 # 注释；文件名须解析在目标目录内",
                       "en": "List format: \"hash  filename\" per line (MD5Sum/SHA256SUM compatible); # comments; filenames must resolve inside the target folder"},
    ...
    "bt_warn_algo_conflict": {
        "zh": "清单扩展名为 {ext}，但其中 {n} 行的哈希长度对应其他算法（如第 {lines} 行），可能混入格式不符的条目。\n\n是否仍按哈希长度自动识别的算法继续比对？",
        "en": "The list extension is {ext}, but {n} line(s) have hash lengths matching other algorithms (e.g. line {lines}), possibly malformed entries.\n\nContinue with the length-detected algorithms anyway?",
    },
    "bt_err_out_of_dir": {
        "zh": "清单中的路径超出目标目录范围（安全边界，已跳过）：{name}",
        "en": "Path in the list escapes the target folder (security boundary; skipped): {name}",
    },
